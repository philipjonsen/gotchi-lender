...
const OWNER_WALLET_ADDRESS =
  process.env.ORIGINAL_OWNER_WALLET_ADDRESS || LENDER_WALLET_ADDRESS
const THIRD_PARTY_WALLET_ADDRESS =
  process.env.THIRD_PARTY_WALLET_ADDRESS ||
    ...
    https.get(POLYGON_GAS_STATION_HOST, (res) => {
      ...
      res.on('end', () => {
        ...
        if (gasData.error) {
          ...
          reject(
            new Error(
              `Polygon gas station response does not include any data for gas speed '${GAS_SPEED}' (rawData=${rawData})`
            )
          )
...
const createAddLendingTransaction = async (
  tokenIds,
  initialCost = UPFRONT_COST_WEI,
  periodSeconds = PERIOD_SECS,
  revenueSplit = [OWNER_SPLIT, BORROWER_SPLIT, THIRD_PARTY_SPLIT],
  originalOwner = OWNER_WALLET_ADDRESS,
  thirdParty = THIRD_PARTY_WALLET_ADDRESS,
  whitelistId = WHITELIST_ID,
  revenueTokens = REVENUE_TOKENS
) => {
  if (tokenIds.length > 1) {
    return createBatchAddLendingTransaction(
      tokenIds,
      initialCost,
      periodSeconds,
      revenueSplit,
...
const createBatchAddLendingTransaction = async (
  tokenIds,
  initialCost = UPFRONT_COST_WEI,
  periodSeconds = PERIOD_SECS,
  revenueSplit = [OWNER_SPLIT, BORROWER_SPLIT, THIRD_PARTY_SPLIT],
...
const createRelistLendingTransaction = async (tokenIds) => {
  if (tokenIds.length > 1) return createBatchRelistLendingTransaction(tokenIds)
  return {
    from: LENDER_WALLET_ADDRESS,
    to: AAVEGOTCHI_DIAMOND_ADDRESS,
    data: contract.methods
      .claimAndEndAndRelistGotchiLending(tokenIds[0])
      .encodeABI()
  }
}
...
const createBatchRelistLendingTransaction = async (tokenIds) => {
  return {
    ...
    to: AAVEGOTCHI_DIAMOND_ADDRESS,
    data: contract.methods
      .batchClaimAndEndAndRelistGotchiLending(tokenIds)
      .encodeABI()
  }
}

const createEndLendingTransaction = async (tokenIds) => {
  if (tokenIds.length > 1) return createBatchEndLendingTransaction(tokenIds)
  return {
    from: LENDER_WALLET_ADDRESS,
    to: AAVEGOTCHI_DIAMOND_ADDRESS,
...
const createCancelLendingTransaction = async (tokenIds) => {
  if (tokenIds.length > 1) return createBatchCancelLendingTransaction(tokenIds)
  return {
    from: LENDER_WALLET_ADDRESS,
    to: AAVEGOTCHI_DIAMOND_ADDRESS,
...
const createBatchCancelLendingTransaction = async (tokenIds) => {
  return {
    ...
    to: AAVEGOTCHI_DIAMOND_ADDRESS,
    data: contract.methods
      .batchCancelGotchiLendingByToken(tokenIds)
      .encodeABI()
  }
}
...
const signTransaction = (unsignedTransaction) =>
  web3.eth.accounts.signTransaction(unsignedTransaction, LENDER_WALLET_KEY)
...
const getGotchisByOwner = (ownerAddress = OWNER_WALLET_ADDRESS) =>
  contract.methods.allAavegotchisOfOwner(ownerAddress).call()
const getIdsFromGotchis = (gotchis) => gotchis.map((g) => g.tokenId)

const getGotchiLendings = (
  ownerAddress = OWNER_WALLET_ADDRESS,
  status = STATUS_AGREED
) =>
  contract.methods
    .getOwnerGotchiLendings(ownerAddress, status, MAX_LENDINGS)
    .call()
const getGotchiLendingListings = (ownerAddress = OWNER_WALLET_ADDRESS) =>
  getGotchiLendings(OWNER_WALLET_ADDRESS, STATUS_LISTED)
const getGotchiIdsFromLendings = (lendings) =>
  lendings.map((l) => l.erc721TokenId)

const isLoanPeriodExpired = (lending) =>
  Math.floor(Date.now() / 1000) >
  parseInt(lending.timeAgreed) + parseInt(lending.period)
...
async function submitTransaction (tokenId, transactionFactory) {
  ...
  try {
    var transaction = await setTransactionGasToMarket(
      await transactionFactory(tokenId)
    )
  } catch (err) {
    return Promise.reject(err)
  }
  log(
    `Creating ${transactionFactory.name} transaction: (tokenId=${tokenId}, from=${transaction.from}, to=${transaction.to}, gasLimit=${transaction.gasLimit}, maxPriorityFeePerGas=${transaction.maxPriorityFeePerGas})`
  )
...
const listGotchiLendings = (tokenIds) =>
  submitTransaction(tokenIds, createAddLendingTransaction).catch((err) =>
    log(`Error creating listing for gotchi IDs ${tokenIds}: ${err.message}`)
  )
const endGotchiLendings = (tokenIds) =>
  submitTransaction(tokenIds, createEndLendingTransaction).catch((err) =>
    log(`Error ending lending for gotchi IDs ${tokenIds}: ${err.message}`)
  )
const relistGotchiLendings = (tokenIds) =>
  submitTransaction(tokenIds, createRelistLendingTransaction).catch((err) =>
    log(`Error relisting lending for gotchi IDs ${tokenIds}: ${err.message}`)
  )
const cancelGotchiLendings = (tokenIds) =>
  submitTransaction(tokenIds, createCancelLendingTransaction).catch((err) =>
    log(`Error canceling lending for gotchi IDs ${tokenIds}: ${err.message}`)
  )

const isGotchiIDOnLendingList = (id) => GOTCHI_IDS.includes(id)

let gotchiIDsListedByScript = []
const gotchiHasBeenListedByScript = (id) =>
  gotchiIDsListedByScript.includes(id)
...
const loop = async () => {
  ...
  const ownedGotchiIds =
    await getGotchisByOwner(OWNER_WALLET_ADDRESS).then(getIdsFromGotchis)
  ...
  const idsOfListedGotchis = await getGotchiLendingListings(
    OWNER_WALLET_ADDRESS
  ).then(getGotchiIdsFromLendings)
  log(`idsOfListedGotchis=${idsOfListedGotchis}`)
  const idsOfGotchisWithLoanPeriodExpired = getGotchiIdsFromLendings(
    lendings.filter(isLoanPeriodExpired)
  )
  log(`idsOfGotchisWithLoanPeriodExpired=${idsOfGotchisWithLoanPeriodExpired}`)
  lendings.forEach((l) =>
    log(
      `tokenId=${l.erc721TokenId}, timeAgreed=${l.timeAgreed}, period=${
        l.period
      }, expireTime=${
        parseInt(l.timeAgreed) + parseInt(l.period)
      }, now=${Math.floor(Date.now() / 1000)}, expired=${isLoanPeriodExpired(
        l
      )}`
    )
  )
  const idsOfUnlistedGotchis = ownedGotchiIds.filter(
    (id) =>
      !idsOfListedGotchis.includes(id) && !idsOfLoanedGotchis.includes(id)
  )
  log(`idsOfUnlistedGotchis=${idsOfUnlistedGotchis}`)
  const idsOfGotchisToList = idsOfUnlistedGotchis.filter(
    isGotchiIDOnLendingList
  )
  log(`idsOfGotchisToList=${idsOfGotchisToList}`)
  const idsOfGotchisToClaimAndEnd = idsOfGotchisWithLoanPeriodExpired
    .filter(isGotchiIDOnLendingList)
    .filter(gotchiHasntBeenListedByScript)
  log(`idsOfGotchisToClaimAndEnd=${idsOfGotchisToClaimAndEnd}`)
  const idsOfGotchisToRelist = idsOfGotchisWithLoanPeriodExpired
    .filter(isGotchiIDOnLendingList)
  ...
  if (CANCEL_ONLY) {
    const idsOfGotchisToCancel = idsOfListedGotchis.filter(
      isGotchiIDOnLendingList
    )
  ...
  if (END_ONLY) {
    const idsOfGotchisToEnd = idsOfGotchisWithLoanPeriodExpired.filter(
      isGotchiIDOnLendingList
    )
    if (idsOfGotchisToEnd.length > 0) endGotchiLendings(idsOfGotchisToEnd)
  }
  ...
  if (!CANCEL_ONLY && !END_ONLY) {
    ...
    if (idsOfGotchisToList.length > 0) {
      ...
      gotchiIDsListedByScript =
        gotchiIDsListedByScript.concat(idsOfGotchisToList)
    }
...
loop().then(() => {
  if (!RUN_ONCE) setInterval(loop, MILLISECONDS_BETWEEN_RETRIES)
})
